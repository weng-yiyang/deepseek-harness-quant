...
执行顺序（依赖链）：
  ...
  1) backfill_delisted   补拉 2019 后退市股（F-2 幸存者偏差）          [需网络/baostock]
  2) fix_st_flags        重拉 isST 标记（F-1 ST 失效 → C5 FAIL）        [需网络/baostock]
...
用法：
  python data/repair_phase1.py                 # 全量修复 + 闸门判定（需你的 token/网络）
...
def main():
    ...
    # 1) 补拉退市股（F-2）
    if not args.skip_network:
        _step("1) 补拉 2019 后退市股（F-2 幸存者偏差）",
              "data/backfill_delisted.py", optional=True)

    # 2) 重拉 ST 标记（F-1）
    if not args.skip_network:
        _step("2) 重拉 isST 标记（F-1 → C5）",
              "data/fix_st_flags.py", optional=True)
